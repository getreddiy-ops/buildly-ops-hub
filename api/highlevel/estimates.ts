...
function normalizeLineItems(body: EstimateInput) {
  ...
  return source
    ...
    .map((item) => ({
      ...
      currency: "USD",
      amount: Math.max(0, Number(item.unit_price) || 0),
  ...
  try {
    ...
    if (req.method === "GET") {
      ...
      if (typeof req.query?.q === "string" && req.query.q.trim()) {
        params.set("search", req.query.q.trim().slice(0, 120));
      }
      if (
      ...
      const result = await highLevelRequest<any>(`/invoices/estimate/list?${params.toString()}`, { token });
      ...
      return json(res, 200, {
        ...result,
        estimates,
        total: Number(result?.total ?? result?.meta?.total ?? estimates.length) || 0,
      });
    }
    ...
    return respondHighLevelError(res, error, "FastTract could not load or save that estimate.");
  }
}
