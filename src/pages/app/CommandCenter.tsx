...
type FunctionErrorContext = { context?: unknown };
type CommandResponse = { error?: string; message?: string; action?: string };
...
export default function CommandCenter() {
  ...
  const run = async (raw?: string) => {
    ...
    if (!raw) setCommand("");
    try {
      const { data, error } = await supabase.functions.invoke<CommandResponse>("fasttract-command", {
      ...
      if (error) {
        ...
        if (context instanceof Response) {
          try {
            ...
            // Keep the original function error when no JSON response is available.
          }
        }
