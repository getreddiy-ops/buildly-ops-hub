...
import { LayoutDashboard, Building2, DollarSign, LogOut, Users, Menu } from "lucide-react";
...
const nav = [
  ...
  { to: "/agent/clients", label: "Client Orgs", icon: Building2 },
  { to: "/agent/leads", label: "Lead Sharing", icon: Users },
  { to: "/agent/payouts", label: "Payouts", icon: DollarSign },
];
  ...
  return (
    <div className="flex min-h-screen w-full bg-background">
      <aside className="hidden w-60 shrink-0 flex-col border-r border-sidebar-border bg-sidebar text-sidebar-foreground md:flex">
        <div className="border-b border-sidebar-border p-4"><Logo to="/agent" /></div>
      ...
      <main className="flex min-w-0 flex-1 flex-col overflow-y-auto">
        <header className="sticky top-0 z-30 flex items-center gap-3 border-b border-border bg-background/95 px-4 py-3 backdrop-blur md:hidden">
          <Sheet open={open} onOpenChange={setOpen} key={location.pathname}>
            ...
            <SheetContent side="left" className="w-72 bg-sidebar p-0 text-sidebar-foreground">
              <SheetHeader className="border-b border-sidebar-border p-4">
                <SheetTitle className="text-left"><Logo to="/agent" /></SheetTitle>
              </SheetHeader>
          ...
          <div className="flex-1 truncate"><Logo to="/agent" /></div>
        </header>
