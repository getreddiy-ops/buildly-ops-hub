...
- Implementation screenshot: `C:\Users\Morgan\Documents\Codex\2026-07-23\wha\work\buildly-ops-hub\implementation-ava-speaking-final.png`
- Combined comparison: `C:\Users\Morgan\Documents\Codex\2026-07-23\wha\work\buildly-ops-hub\design-comparison-ava-speaking-final.png`
...
- State: first conversational business question, Ava actively speaking, microphone not consented

## Full-view comparison evidence

The final comparison shows the same major composition as the selected target: realistic Ava portrait in the upper region, visible speaking mouth pose, dark warm FastTract palette, explicit AI label and state, one clear question, voice/text response controls, and secondary navigation below. The implementation shows Ava speaking while text entry remains available; the source shows the listening reply state. Both modes are implemented.
...
- Image quality: three aligned, dedicated photorealistic Ava phoneme frames are used; crop is sharp and consistent on mobile and desktop. No placeholder, CSS-drawn, or deceptive prerecorded avatar is used.
...
4. User feedback found a P1 believability issue: Ava’s speaking state still looked like a static portrait. Two additional real portrait assets were generated for rounded “oh” and wide “ee” mouth poses. The speaking engine now advances those visemes while audio plays, with boundary-driven updates when the browser exposes speech boundaries and a timed fallback when it does not.
5. Post-fix evidence: `design-comparison-ava-speaking-final.png` shows Ava in a clear mid-speech pose with the visible SPEAKING indicator. Two captures taken 170 ms apart show distinct “ee” and “oh” mouth shapes. No browser console errors were recorded.
...
- P3: a production streaming-avatar provider can later replace the local viseme sequence with audio-derived facial animation while preserving the current consent and fallback behavior.
